...
class PostCreate(PermissionRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['post_category'] = Post.objects.all
        return context
...
# @permission_required(['app.change_response'])
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
        # my_posts = Post.objects.filter(post_author = request.user)
            context['count'] = Response.objects.filter(response_post__in=Post.objects.filter(post_author = self.request.user)).values("response_post_id").distinct()
            context['res'] = Response.objects.filter(response_post__in=Post.objects.filter(post_author = self.request.user))
            context['myposts'] = Post.objects.filter(post_author = self.request.user).order_by('-created_date').all()
            context['page'] = self.request.GET.get('page')
            
            context['my_pages'] = Paginator(context['myposts'], per_page=self.paginate_by).get_page(context['page'])
                            # print(Paginator(qs,5).get_page(page))
            return context
